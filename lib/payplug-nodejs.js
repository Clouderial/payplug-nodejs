...
PayPlugAPI.prototype.authenticate = function () {
	...
	this.doGet('/v1/payments')
		.then(function (res) {
			log.info('We are successfully connected to PayPlug Server at "%s"', this.options.payplugUrl);
			this.authenticated = true;
			deferred.resolve();
		})
		.fail(function (res) {
			log.error('Cannot authenticate against PayPlug server at "%s" cause "%s" (%s)', this.options.payplugUrl, res.statusCode, res.statusMessage);
			this.authenticated = false;
			deferred.reject();
		})
		.done();
...
PayPlugAPI.prototype.doSend = function (method, path, body) {
	...
	var cb = function (res) {
		...
		log.info(' <-- GET "%s" : status="%s" (%s)', path, res.statusCode, res.statusMessage);
