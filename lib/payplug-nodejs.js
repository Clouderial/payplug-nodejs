...
PayPlugAPI.prototype.doSend = function (method, path, body) {
	...
	if (method === 'POST' || method === 'PUT') {
        ...
        httpOptions.headers['Content-length'] = bodyStr.length;
	}
    ...
    var cb = function (res) {
		...
		res.on('data', function(chunk){
			log.trace('Receive body response : "%s"', chunk);
			body += chunk;
		});
		res.on('end', function() {
    		log.trace('No more data in response. Body is "%s"', body);
//			log.trace('Response : "%s"', util.inspect(res));
			log.info(' <-- %s "%s" : status="%s" (%s) : %s', method, path, res.statusCode, res.statusMessage, body);
    ...
    if (!_.isEmpty(body)) {
        log.trace('Post the body into request');
        req.write(bodyStr);
    }
