...
var PayPlugAPI = function (secretKey, options) {
    ...
    this.options = _.extend({
        ...
        sucessReturnUrl: 'http://myserver/success?tracker=',
        cancelReturnUrl: 'http://myserver/cancel?tracker=',
        notificationUrl: 'http://myserver/notifications?tracker=',
    }, options);
...
PayPlugAPI.prototype.doSend = function (method, path, body) {
    ...
    log.trace('We will Do Get with "%s"', util.inspect(httpOptions));
    log.info(' --> %s "%s"', method, path);
    var req;
    var cb = function (res) {
        log.trace('Response : "%s"', util.inspect(res.req.domain));
        log.info(' <-- %s "%s" : status="%s" (%s)', method, path, res.statusCode, res.statusMessage);
        if (res.statusCode >= 200 && res.statusCode < 400) {
            ...
            if (res.statusCode > 499) {
                return deferred.reject(new jmcnetException.TechnicalException(res.statusCode, [ res.statusMessage, res.req.method+' '+res.req.path]));
            }
            else {
                return deferred.reject(new jmcnetException.FunctionalException(res.statusCode, [ res.statusMessage, res.req.method+' '+res.req.path]));
            }
        }
    };
